...
#################Species tree##################
##LOAD DATA##
load("ThreeStates.joint.RData")
#temp
tips <- as.data.frame(tree$tip.label)
length(tree$tip.label)
###For taxtree
write.tree(tree,file="ThreeStateTree.notax.phy",tree.names = FALSE)
sp.taxtree=read.tree("Make_TaxTree/ThreeStates.tax.phy")
length(sp.taxtree$tip.label)
#Node states
sp.joint.node.states <- as.data.frame(ThreeStates.bestML.joint$states)
...
sp.joint.node.states$node <- sp.joint.node.states[, 1] + length(tree$tip.label)
#Tip states
ThreeStates.bestML.joint$tip.states
sp.joint.tip.states <- as.data.frame(ThreeStates.bestML.joint$tip.states)
...
#(1,R1) is the precursor state
#(2) is actinorhizal
#(3) is rhizobial
#order:(1,R1)      (2,R2)      (3,R1)       (1,R2)     (2,R1) (3,R2)
#red,green,blue,gray,green,blue
cols=c("orange","purple","blue","gray64","purple","blue")
...
p.lin <- ggtree(tree,ladderize=TRUE,continuous="none",size=.1) %<+% 
  sp.joint.all.states + aes(color=states)+scale_color_manual(values=cols)

#Look at solution
ThreeStates$solution
reorder.solution <- ThreeStates$solution
rownames(reorder.solution) <- rownames(ThreeStates$index.mat)
colnames(reorder.solution) <- colnames(ThreeStates$index.mat)
reorder.solution
#Gain of actinorhizal is 10x slower than loss of actinorhizal
#Gain or rhiozbial from actinorhizal is 5x faster than gain of actinorhizal from precursor
write.csv(file="solution.csv",reorder.solution)
...
p.circ.col=p.circ.base+scale_colour_manual(values=cols)
...
ggsave(height = 10,width=6,filename="ExtendedData.Fig3.pdf",dpi=300)
ThreeStates$solution
ThreeStates$index.mat
#
###FIGURE:Circle tree. Rosids with non nit-fix collapsed, two colors ###
#Add color to whole tree or nitfix clade
