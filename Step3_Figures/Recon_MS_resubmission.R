...
all.rates=c("red","purple","darkorange1","green","gray64","blue")

#Supplementary Data
p.lin <- ggtree(sp.taxtree,ladderize=TRUE,continuous="none",size=.1) %<+% sp.joint.all.states + aes(color=states)+scale_color_manual(values=all.rates)
p.lin.lab=p.lin+geom_tiplab(size=.2)
NFC=viewClade(p.lin.lab,N.MRCA)
NFC
ggsave(height = 120,width=6,filename="SupplementaryData.Resubmission.acceptednames.pdf",dpi=300,limitsize=FALSE)

#6/5/2023
#Add sequencing_Well to tip label
NFC_tips <- na.omit(data.frame(NFC[["data"]][["label"]]))
temp_well_label <- cbind(tips,NFC_tips)
NFC_relabeled[["data"]][["label"]] <- c(paste(temp_well_label$`v7.joint.bestML.edit.v2$phy$tip.label`,temp_well_label$NFC...data......label...,sep="."),rep(NA, 24372))
write.tree(as.phylo(NFC_relabeled), file = "NFC_tree.nwk")
